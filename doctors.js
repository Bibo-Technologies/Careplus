...
const tableHeaderRow = document.createElement('tr');
const headers = ['Medication', 'Prescription', 'Mg', 'Cost'];
headers.forEach((headerText) => {
  const tableHeaderCell = document.createElement('th');
  tableHeaderCell.textContent = headerText;
  tableHeaderRow.appendChild(tableHeaderCell);
...
addRecordForm.addEventListener('submit', function (e) {
  ...
  const testsTaken = selectedTestOption ? selectedTestOption.value : '';
   ...
   // Extract the price from the selected test option
   const priceMatch = selectedTestOption.value.match(/Price: UGX (\d+\.\d+)/);
  ...
  const recordData = {
    ...
    price: price, // Add the price to the record data
